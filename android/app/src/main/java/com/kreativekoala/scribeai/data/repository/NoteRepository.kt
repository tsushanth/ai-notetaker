...
class NoteRepository {
    ...
    suspend fun generateInfographic(
        ...
        return withContext(Dispatchers.IO) {
            try {
                val request = GenerateAIRequest(noteId, "infographic", options)
                val response = apiService.generateInfographic("Bearer $token", request)
                    ...
                    val errorMessage = when (response.code()) {
                        ...
                        403, 402 -> {
                            when {
                                errorBody.contains("subscription", ignoreCase = true) ||
                                errorBody.contains("premium", ignoreCase = true) ->
                                    "Infographic generation requires a premium subscription."
                                errorBody.contains("trial", ignoreCase = true) ->
                                    "Your free trial has ended. Subscribe to continue."
                                else -> "Access denied. Please check your subscription status."
                            }
                        }
                        else -> "Failed to generate infographic. Please try again."
                    }
                    Log.e(TAG, "❌ generateInfographic failed: ${response.code()} - $errorBody")
                    Result.failure(SubscriptionException(errorMessage, response.code()))
                }
            } catch (e: Exception) {
                Log.e(TAG, "❌ generateInfographic exception", e)
                Result.failure(e)
            }
        }
    }
